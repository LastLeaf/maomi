...
fn component_lifetimes() {
    ...
    context.set_root_component(root_component);
    let root_component = context.root_component::<LifetimeParentComponent<Empty>>().unwrap();
    {
        let root_component = root_component.borrow_mut();
        let mut html: Vec<u8> = vec![];
        root_component.to_html(&mut html).unwrap();
        assert_eq!(std::str::from_utf8(&html).unwrap(), r#"<maomi>attached<maomi-lifetime-child-component>attached</maomi-lifetime-child-component><maomi-lifetime-child-component>attached</maomi-lifetime-child-component></maomi>"#);
    }
    let root_component = root_component.borrow_mut();
    let mut html: Vec<u8> = vec![];
    root_component.to_html(&mut html).unwrap();
