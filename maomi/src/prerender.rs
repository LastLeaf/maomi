...
pub(crate) fn match_prerendered_tree<B: Backend>(root: &mut ComponentNodeRefMut<B>, backend: &Rc<B>) {
    ...
    fn match_prerendered_children<B: Backend>(children: &mut ChildIterMut<B>, child_iter_state: &mut ChildIterState<B>) {
        while let Some(mut child) = children.next() {
            {
                ...
                match &child_iter_state.prev {
                    NodeRc::NativeNode(n) => {
                        ...
                        match_prerendered_children(&mut c, &mut ChildIterState { prev, next_is_child: true });
                    },
    ...
    match_prerendered_children(&mut c, &mut ChildIterState { prev: root_rc, next_is_child: true });
}
