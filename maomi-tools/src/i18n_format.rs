    ...
    for ns in namespaces {
        ...
        for item in trans_items {
            ...
            writeln!(
                w,
                r#""{}" = "{}""#,
                toml_str_escape(item.src),
                toml_str_escape(item.translated)
            )?;
        }
    }
    ...
    maomi_tools::config::crate_config(|crate_config| {
        ...
        let locale = {
            cmd_args.locale.as_ref().unwrap_or_else(|| {
                crate_config.i18n_locale.as_ref().expect(
                    "locale not specified (try specify `MAOMI_I18N_LOCALE` environment variable)",
                )
            })
        };
        let i18n_dir = crate_config
            .i18n_dir
            .as_ref()
            .expect("no proper i18n directory found");

        // read metadata and original translation file
        let src_path = i18n_dir.join(format!("{}.toml", locale));
        let format_metadata_path = i18n_dir
            .join("format-metadata")
        ...
        let format_metadata: FormatMetadataOwned =
            toml::from_str(&format_metadata).expect("illegal format metadata");
        ...
        do_format(
            ...
            format_metadata,
            src,
            cmd_args.missing.as_ref().map(|x| x.as_str()),
        )
        .unwrap();
        let formatted = r;
