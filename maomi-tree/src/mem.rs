use std::{pin::Pin, mem::{ManuallyDrop, MaybeUninit}, rc::Rc, cell::{Cell, RefCell, UnsafeCell}, ptr};
...
impl<T, const N: usize> SliceAlloc<T, N> {
    pub(crate) fn new() -> Self {
        let inner = SliceAllocInner { slices: vec![], last_freed: ptr::null(), last_used_count: N };
    ...
    pub(crate) fn alloc(&mut self, data: T) -> SliceRc<T, N> {
        ...
        let mut mem = if !inner.last_freed.is_null() {
            ...
            let r = unsafe { &mut inner.slices.last_mut().unwrap().slices.as_mut().get_unchecked_mut()[i] };
            r
        };
...
mod test {
    ...
    fn reuse() {
        ...
        let d10 = sa.alloc(10);
        let d11 = sa.alloc(11);
        assert_eq!(d11.mem() as usize - d10.mem() as usize, std::mem::size_of::<SliceInner<usize, 3>>());
        let (d12mem, d13mem) = {
            let d12 = sa.alloc(12);
            let d13 = {
